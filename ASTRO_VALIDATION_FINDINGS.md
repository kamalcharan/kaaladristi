...
| 9 | Astro times **trend inflections** (does a break *change* the trend vs *accelerate* it — direction-agnostic, pure OHLC, the sharpest hypothesis) | **NULL — not robust** | Mercury-adjacent break change-rate flips across decades: 2000s 3.6% vs 9.2% baseline (lower), 2010s 8.8 vs 8.0, 2020s 11.4 vs 7.8 — modern lean inside noise, 2000s contradicts. Robust finding underneath is *technical*: ~92% of 10-day-range breaks accelerate the trend (change-rate ~8% every decade, stable). |
