-- ============================================================================
-- km_migration_195_price_action_matview.sql
-- ----------------------------------------------------------------------------
-- Moves the six price-action scanners onto km_scan_results:
--
--   breakout_surge, weekly_movers, monthly_movers,
--   weekly_decliners, monthly_decliners, breakdown_watch
--
-- They were Path B direct-query fetchers, each with its own hand-written
-- PostgREST SELECT in scanEngine.ts. Migration 147 excluded breakout_surge by
-- name ("OUT OF SCOPE (Path B direct-query, MUST NOT regress)"), and the five
-- built in migrations 183-189 followed that pattern. That hand-written select
-- list is the whole reason the Columns picker showed dashes: fieldAvailability
-- offers columns per CATEGORY, each fetcher chose its own subset, and nothing
-- reconciled the two. A preset served from this view has no select list to
-- get wrong -- every column the UI can render is on the row.
--
-- Supersedes migration 181 (each matview migration recreates the view whole).
-- The nine existing arms are carried over byte-identical apart from the eight
-- appended price-action columns, so they need no re-parity.
--
-- NEW COLUMNS on km_scan_results (migrations 183/185/187, already in
-- km_equity_eod and already flowing through eq_base's SELECT e.* -- they were
...
--
-- PARITY -- one deliberate narrowing. eq_base gates on ema_20 IS NOT NULL, as
-- every matview preset already does; the six direct fetchers did not. Measured
-- on 2026-08-27 (NSE, close >= 50):
--   weekly_movers    1098 -> 1052     monthly_movers    1246 -> 1246
--   weekly_decliners 1387 -> 1324     monthly_decliners 1142 -> 1142
--   breakout_surge    213 ->  199     breakdown_watch    328 ->  309
-- The four period scans cap at 500 either way, so only breakout_surge and
-- breakdown_watch change on screen -- 14 and 19 rows, all young listings with
-- no 20-day EMA yet. km_scan_exclusion_counts now covers these six so the
-- number is visible rather than inferred.
--
-- KNOWN DATA GAP -- breakdown_level / pct_from_breakdown (migration 187) are
-- populated for ~465 rows a day before 2026-08-27 and fully only from that
-- date, because the history backfill has not been run. Breakdown Surge filters
-- on pct_from_breakdown < 0 and NULL fails that test, so the scan was reading
-- ~6% of the universe until the 27th. This migration does not change that --
-- scripts/backfill_rolling_metrics_fast.py still needs to run for history.
--
-- Target DB: kaala_dristi_db. Run in pgAdmin / DBeaver / psql.
