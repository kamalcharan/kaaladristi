-- ============================================================================
-- km_migration_200_big_money_days.sql
-- Target DB: kaala_dristi_db
--
-- BIG MONEY DAYS -- moved from client-side detection to stored columns, and
-- surfaced on km_scan_results so scanners, ChartView, bookmarks and the VaNi
-- intents all read ONE number instead of each deriving their own.
--
-- WHAT A BIG MONEY DAY IS (unchanged in substance from services/bigMoney.ts)
--
--   A session where DELIVERED value -- shares actually taken home, not
--   day-traded -- ran far above the stock's own norm. It marks a price zone
--   where large money changed hands. It is an OBSERVATION, never a support
--   claim: delivery is two-sided by definition, so "direction" is inferred
--   from how price absorbed the handover, not asserted.
--
-- THREE GATES, all self-relative (no mcap lookup, no flat rupee floor):
--
--   1. bm_ratio  = deliv_value_cr / avg_amt_66d >= 5
--      avg_amt_66d IS the stock's 66-day delivered-value norm and is already
--      computed by the rolling-metrics step -- no rolling window needed here.
--      MIN_RATIO 5 is the owner calibration from 2026-07-07, anchored by TARIL
--      19-Jun-2026 (Rs 240 Cr delivered vs a Rs 40 Cr baseline = 6.0x).
--
--   2. deliv_value_cr must land in the stock's own top 2% of the PRIOR up-to-252
--      sessions. The client computed this percentile over whatever window the
--      chart happened to have loaded, so the same stock produced different Big
--      Money days at 6M and 1Y. Fixed here at a trailing 252 bars, PRIOR-ONLY,
--      so a past bar never repaints from data that came after it.
--
--   3. deliv_value_cr >= Rs 1 Cr -- a sanity floor so near-zero-delivery names
--      cannot fire on noise. Without it, ratio-alone gives a tail of 33
--      events/year on stocks whose delivery baseline is tiny and erratic
--      (measured over the trailing year: 4,414 stocks, avg 5.5 events each).
--
-- TWO STORED COLUMNS, and deliberately no more:
--
--   bm_event  'entry' | 'exit' | 'mixed' | NULL   -- sparse, only on event bars
--   bm_ratio  delivered value / own 66-day norm   -- written on every bar that
--             has a baseline, so it doubles as the backfill's resume marker
--             (the same role gl_days_above plays for migration 194).
--
--   The ZONE needs no column: it is the event bar's own low/high.
--   heldAbove / sessionsSince are NOT stored -- they change every session for
--   every past event, so storing them would mean rewriting all history nightly.
--   They are read-time arithmetic.
--
-- MATVIEW ROLLUP -- eight columns appended to km_scan_results
--
-- km_scan_results is a LATEST-DATE view (one row per stock per preset), so it
-- cannot hold event history. What it carries is the rollup a scanner or a card
-- actually filters on: where the last Big Money day was, which way it read,
-- and whether price still sits above that zone.
--
--   bm_last_date, bm_last_event, bm_last_ratio, bm_last_low, bm_last_high,
--   bm_days_since, bm_above_last_zone, bm_count_252d
--
-- The fifteen preset arms are UNCHANGED -- byte-identical to migration 197.
-- Rather than append eight columns to fifteen UNION branches, the whole union
-- is wrapped in a `u` CTE and the rollup is LEFT JOINed once at the outer
-- SELECT. The rollup is per-EQUITY, not per-preset, so joining it fifteen
-- times would have been fifteen chances to get one branch wrong.
--
-- DATA HORIZON -- read this before judging coverage
--
-- Delivery data does not exist in km_equity_eod before 2024-06-03 (NSE) /
-- 2024-07-10 (BSE) -- delivery_qty is 0, not merely NULL. Big Money is defined
-- on delivered value, so it CANNOT be computed before those dates no matter
-- what the backfill does. Deeper history needs the delivery feed backfilled
-- first (scripts/backfill_nse_delivery.py, scripts/backfill_bse_delivery.py),
-- then rolling metrics recomputed, then this backfill re-run -- it is
-- idempotent and simply extends.
--
-- Supersedes migration 197.
--
-- After COMMIT, repopulate (order matters -- see the tail of this file).
-- ============================================================================

-- ---------------------------------------------------------------------
-- (1) km_equity_eod columns -- OUTSIDE the transaction, deliberately.
--
-- Same polled-ALTER pattern as migration 194 and for the same reason: ACCESS
-- EXCLUSIVE on km_equity_eod needs every other lock released at the same
-- instant, and a plain ALTER that waits also blocks every reader queued behind
-- it -- which stops looking like a lock and starts looking like an outage.
-- Each attempt gives up after 2 seconds and releases the queue.
--
-- Exhausting 200 attempts (~10 min) is a FINDING, not a longer wait: something
-- holds the lock continuously. Find it with pg_locks joined to pg_stat_activity
-- on 'km_equity_eod'::regclass -- a running backfill, the pipeline scheduler,
-- or a client left in an open transaction.
--
-- No BEGIN here: the retry loop must run in autocommit so each attempt
-- actually releases what it took.
-- ---------------------------------------------------------------------
SET lock_timeout = '2s';
  ...
  LOOP
    ...
    BEGIN
      ALTER TABLE km_equity_eod
          ADD COLUMN IF NOT EXISTS bm_event VARCHAR(8),
          ADD COLUMN IF NOT EXISTS bm_ratio NUMERIC(10,2);
      RAISE NOTICE 'km_equity_eod: bm columns added (attempt %)', tries;
      EXIT;
    EXCEPTION WHEN lock_not_available THEN
      IF tries >= 200 THEN
        RAISE EXCEPTION
          'km_equity_eod still locked after % attempts (~10 min). Something is holding a long transaction -- find it with pg_locks/pg_stat_activity rather than waiting.', tries;
      END IF;
      IF tries % 20 = 0 THEN
...
COMMENT ON COLUMN km_equity_eod.bm_event IS
    'Big Money day footprint: entry = up bar closing in the top 40% of its range; exit = down bar closing in the bottom 40%; mixed = large handover with no clear price verdict. NULL on every non-event bar.';
COMMENT ON COLUMN km_equity_eod.bm_ratio IS
    'Delivered value (deliv_value_cr) as a multiple of this stock''s own 66-day delivered norm (avg_amt_66d). Written on every bar with a baseline; >= 5 is one of the three Big Money gates.';

RESET lock_timeout;

-- ---------------------------------------------------------------------
-- (2) The matview rebuild. Its own transaction: a lock timeout in part (1)
-- must not cost this, and vice versa.
-- ---------------------------------------------------------------------
...
-- ══ migration 200: the fifteen arms below are UNCHANGED from migration 197 ══
-- They are wrapped in `u` so the Big Money rollup can be attached once, at the
-- outer SELECT, instead of appended to fifteen UNION branches.
u AS (
...
-- ══ Outer projection: the fifteen arms + the Big Money rollup ═══════════════
...
--   GROUP BY 1,2;
-- The fifteen arms are byte-identical to migration 197 — no re-parity needed.
--
-- BIG MONEY ORDERING — the rollup columns read km_equity_eod.bm_event, which
-- nothing has written yet at this point. Expect them all NULL on the first
-- refresh; that is correct, not a bug. Populate them with:
--   cd App/backend && python scripts/backfill_big_money.py
--   python scripts/backfill_big_money.py --verify
-- then REFRESH km_scan_results again. From the next nightly run the
-- `big_money` pipeline2 dimension keeps them current.
-- ============================================================================
