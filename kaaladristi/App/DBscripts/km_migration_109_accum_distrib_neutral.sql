-- ============================================================
-- Migration 109 · A/D full coverage — add NEUTRAL label
--
-- Problem: compute_flow_intelligence() section 3 only wrote
-- ACCUMULATION or DISTRIBUTION, leaving all other equity-days
-- as NULL. NULL is ambiguous — it could mean "no signal" or
-- "data gap". In the scanner UX this makes the column useless
-- at 34% coverage (66% blank cells look like missing data).
--
-- Fix: when SMA 150 is available but neither ACCUMULATION nor
-- DISTRIBUTION condition fires, write 'NEUTRAL'. This gives
-- full coverage for all stocks with SMA 150 computed (~95%+).
-- NULL is now reserved strictly for true data gaps where
-- SMA 150 has not yet been computed.
--
-- Signal semantics:
--   ACCUMULATION — price below SMA 150 + bullish momentum/RS
--   DISTRIBUTION — price above SMA 150 + bearish momentum/RS
--   NEUTRAL      — SMA 150 available, no contested A/D regime
--   NULL         — SMA 150 not computed (true data gap)
--
...
BEGIN
  ...
  FOR i IN start_idx..n LOOP
    ...
    -- NEUTRAL is written when SMA 150 is available but no A/D regime fires,
    -- so the column has full coverage for all stocks with SMA 150 computed.
    -- NULL is reserved for true data gaps (SMA 150 not yet computed).
    ...
    ELSIF a_sma_150[i] IS NOT NULL THEN
      a_accum_distrib[i] := 'NEUTRAL';
    -- else: SMA 150 not computed — leave NULL (true data gap)
    END IF;
