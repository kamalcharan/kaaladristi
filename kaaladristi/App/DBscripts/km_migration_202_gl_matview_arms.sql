-- ============================================================================
-- km_migration_202_gl_matview_arms.sql
-- Target DB: kaala_dristi_db
--
-- GOLDEN LINE PAIR ONTO km_scan_results, and the GL / Big Money / dot bar
-- columns projected for every arm.
--
-- Gap audit 2026-09-06 §3a / task C1 (docs/claude/scanner-gap-audit-2026-09-06.md):
-- gl_breakout and gl_retest were the only Price Action presets served by a
-- hand-written fetcher (scanEngine.ts fetchGlEvents) instead of the matview.
-- Everything the matview gives its fifteen arms, the pair lacked: the Columns
-- picker showed dashes for xamt / rel_* / magic_rs_trend / the week-and-month
-- pairs, there were no history_insufficient / guard_notes audit columns, the
-- tab strip had to count them by running the fetcher, and the Discovery board
-- read them with two extra queries. The reverse gap was real too: no arm
-- carried pct_from_gl / gl_event / gl_days_above, so "vs GL" and "GL Event"
-- were dashes on Breakout Surge and every other matview preset.
--
-- WHAT CHANGES (and only this):
--
--   1. `stock` projects eight more km_equity_eod columns on the latest bar:
--        pct_from_gl, gl_event, gl_days_above   (migration 194)
--        bm_event, bm_ratio                      (migration 200)
--        dot_svd, dot_sbd, dot_syd               (the bar's OWN dots — the
--                                                 has_recent_* trio is the
--                                                 5-session window, a
--                                                 different statement; the
--                                                 GL table's Dot column is
--                                                 the bar's)
--      They ride into every arm derived from `stock`; the Flower Pot arm
--      (own base) carries typed NULLs in the same positions.
--
--   2. Two new arms, 'gl_breakout' and 'gl_retest', mirroring fetchGlEvents:
--        NSE only, one row per ISIN (NSE preferred), gl_event = the preset's
--        event, NO close >= 50 gate (the pair never had one — a Rs 30 stock
--        reclaiming its Golden Line is exactly what the scan is for), cap 200.
--        Breakouts rank by pct_from_gl DESC (freshest reclaim first), retests
--        by gl_days_above DESC (longest hold first) — the fetcher's own order.
--        vani_flag is TRUE on every row: the preset's vani_rule is
--        gl_event_any, and every row IS a Golden Line event on a volume-drive
--        or accumulation bar, which is what the owner defined the highlight as.
--      One gate the fetcher did not have: `stock` requires ema_20 IS NOT NULL.
--      It cannot bite here — gl_event needs sma_150, which needs 150 bars,
--      while ema_20 needs 20 — but it is named so nobody hunts for it.
--      Parity measured on the live DB while writing this (2026-09-04 bar):
--      17 breakouts / 2 retests from the arm logic, identical to the fetcher.
--
--   3. km_scan_exclusion_counts enumerates the two new presets.
--
-- The fifteen existing arms are otherwise byte-identical to migration 200.
-- Column ORDER is unchanged (new columns appended at the end of `u`), so
-- every SELECT * consumer keeps its shape.
--
-- Frontend side (same commit): the pair's descriptor is source: 'matview';
-- executeScan tries the arm first and falls back to fetchGlEvents while this
-- migration is unapplied (empty arm = the signal), and the tab count does the
-- same. scanRowToScanStock maps the new columns. The nightly integrity check
-- derives "matview-served" from that descriptor, so between deploying the
-- frontend and running this migration it will report the pair's arm as
-- EMPTY — apply and refresh promptly after deploy, or read that finding as
-- "202 not run yet".
--
-- Supersedes migration 200's view definition.
--
-- After COMMIT, repopulate (see the tail of this file).
-- ============================================================================

-- ---------------------------------------------------------------------
-- The matview rebuild. One transaction; no table ALTERs in this migration
-- (every column it reads already exists: gl_* from 194, bm_* from 200,
-- dot_* from the compute_dots step).
-- ---------------------------------------------------------------------
...
SELECT 'flower_pot_burst', rnk::int, is_burst AS vani_flag,
       ...
       NULL::numeric, NULL::numeric, NULL::numeric, NULL::numeric,
       NULL::numeric, NULL::text, NULL::int, NULL::text, NULL::numeric, NULL::boolean, NULL::boolean, NULL::boolean
FROM fpb
