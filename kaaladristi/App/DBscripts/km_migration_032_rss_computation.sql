-- ============================================================
-- Migration 032 · Implement RSS (Relative Strength Spread)
--
-- RSS was declared but never computed — rss_value was always
-- NULL, and rss_rsi was just a copy of rsi_14.
--
-- RSS formula (matches LuckyPop Pine Script):
--   E1 = SMA(close, 10)
--   E2 = SMA(close, 40)
--   Spread = E1 - E2
--   RSS_raw = RSI(Spread, 5)   ← Wilder's smoothing via _wilder_ema
--   rss_value = SMA(RSS_raw, 3) ← 3-bar smooth
--
-- New columns: sma_10, sma_40, rss_spread, rss_value
-- Minimum bars needed: 40 (SMA_40) + 5 (RSI warm-up) + 3 (smooth) = 48
--
-- rss_rsi is no longer written (was a broken copy of rsi_14).
-- ============================================================

-- ── Schema: add columns to both EOD tables ─────────────────
ALTER TABLE km_index_eod  ADD COLUMN IF NOT EXISTS sma_10 FLOAT8;
ALTER TABLE km_index_eod  ADD COLUMN IF NOT EXISTS sma_40 FLOAT8;
ALTER TABLE km_index_eod  ADD COLUMN IF NOT EXISTS rss_spread FLOAT8;

ALTER TABLE km_equity_eod ADD COLUMN IF NOT EXISTS sma_10 FLOAT8;
ALTER TABLE km_equity_eod ADD COLUMN IF NOT EXISTS sma_40 FLOAT8;
ALTER TABLE km_equity_eod ADD COLUMN IF NOT EXISTS rss_spread FLOAT8;

-- rss_value already exists (was always NULL). No ALTER needed.

COMMENT ON COLUMN km_index_eod.sma_10     IS 'SMA(close, 10) — used for RSS spread';
COMMENT ON COLUMN km_index_eod.sma_40     IS 'SMA(close, 40) — used for RSS spread';
COMMENT ON COLUMN km_index_eod.rss_spread IS 'RSS spread = SMA_10 - SMA_40';
COMMENT ON COLUMN km_index_eod.rss_value  IS 'RSS = SMA(3) of RSI(spread, 5). Relative Strength Spread oscillator.';

COMMENT ON COLUMN km_equity_eod.sma_10     IS 'SMA(close, 10) — used for RSS spread';
COMMENT ON COLUMN km_equity_eod.sma_40     IS 'SMA(close, 40) — used for RSS spread';
COMMENT ON COLUMN km_equity_eod.rss_spread IS 'RSS spread = SMA_10 - SMA_40';
COMMENT ON COLUMN km_equity_eod.rss_value  IS 'RSS = SMA(3) of RSI(spread, 5). Relative Strength Spread oscillator.';


-- ── Patched compute_indicators_batch ───────────────────────
...
BEGIN
  ...
  -- Formula (LuckyPop Pine Script):
  --   E1 = SMA(close, 10)   → a_sma_10 (computed above)
  --   E2 = SMA(close, 40)   → a_sma_40 (computed above)
  --   Spread = E1 - E2
  --   RSS_raw = RSI(Spread, 5) using Wilder's smoothing
  --   rss_value = SMA(RSS_raw, 3) — 3-bar smooth
  a_rss_spread := array_fill(NULL::FLOAT8, ARRAY[n]);
  a_rss_value  := array_fill(NULL::FLOAT8, ARRAY[n]);

  -- Build spread array (first valid at index 40 where both SMAs exist)
  FOR i IN 40..n LOOP
  ...
  -- RSI of spread (period 5, Wilder's smoothing)
  DECLARE
  ...
  BEGIN
    -- Build spread gain/loss (NULL before index 41, 0 or positive after)
    FOR i IN 41..n LOOP
    ...
    -- _wilder_ema skips NULLs, so indices 1-40 are ignored.
    -- First output at index ~45 (5 non-null values from 41 onward).
    ...
    FOR i IN 1..n LOOP
      ...
      ELSIF v_sg5[i] IS NOT NULL AND v_sl5[i] IS NOT NULL AND v_sg5[i] > 0 THEN
        a_rss_raw[i] := 100;  -- all gains, no losses
      END IF;
    END LOOP;

    -- SMA(3) smoothing → rss_value
    -- First valid at ~index 47 (3 consecutive non-null rss_raw values)
    FOR i IN 3..n LOOP
