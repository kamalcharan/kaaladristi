-- ============================================================================
-- km_migration_174_waking_giants_scan.sql
-- ----------------------------------------------------------------------------
-- Waking Giants + First Ascent scan presets (POA: docs/claude/waking-giants-poa.md,
-- spec: docs/claude/scanner-audit-2026-07-12.md par.6b/7/8).
--
-- Re-creates km_scan_results with the migration-170 body UNCHANGED plus:
--
--   * ONE shared WG CTE chain (wg_const .. wg_scored) computing every gate
--     with listing_age_years as a column, and TWO banded preset blocks
--     ('waking_giants' >= 10y, 'first_ascent' 6-10y) — the owner's
--     code-reuse contract: one engine, two age bands, no user dropdown.
--   * Five columns appended to every UNION block (NULL outside WG):
--     listing_age_years, pct_from_3y_high, days_since_3y_high, gl_acc_days,
--     wg_phase.
--   * kd_scan_presets rows for both presets (SEBI-observational copy, D39).
--
-- GATES (constants named in wg_const; dormancy calibrated 2026-08-24 from the
-- cliff-adjusted compute_dormancy.py report — Giants land at ~142, First
-- Ascent ~27 pre-phase, inside the audit's 100-150 watchlist target):
--   age >= 6y (banded at 10y) . mcap_cr >= 200 . ADV(22) >= 1 Cr
--   . DORMANT: <= -60% from a >= 1-yr-old cliff-adjusted 3-yr high,
--              OR 3-yr high/low ratio <= 1.8 (long flat range)
--
-- PHASES (observational, logic-only — surfaced strings stay D39-safe):
--   WAKING   — >= 12 GL accumulation days in the last 60 sessions AND
--              Magic RS above its 22-session-ago value while price is
--              still within +/-10% (RS pushing before price)
--   STIRRING — >= 6 GL accumulation days
--   DORMANT  — on the watchlist, no accumulation evidence yet
--   GL day = delivery_pct >= 55 AND |pct_chng| <= 2 AND rvol <= 2.5
--   (delivery-backed, quiet, non-explosive). GL thresholds are v1
--   estimates — calibrate against distributions once rows flow.
--
-- DEPENDS ON: migration 172 (mcap freshness) + 173 (dormancy metrics) with
-- compute_dormancy.py run at least once — wg_pool requires
-- pct_from_3y_high IS NOT NULL, so an un-run 173 yields empty presets, not
-- errors.
--
-- OPERATIONAL NOTES (same as 170):
--   * Run as SUPERUSER (or vikuna_admin) — DROPs matviews owned by kd_app;
--     ownership is re-issued to kd_app at the bottom.
--   * Brief absence window between DROP and CREATE WITH NO DATA; run
--     off-hours. REFRESH both views after COMMIT (results FIRST).
-- ============================================================================
...
-- ══ Waking Giants / First Ascent — shared engine (POA: waking-giants-poa.md) ══
-- ONE CTE chain computes every gate with listing_age_years as a column; the
-- two preset blocks below band it. All constants are NAMED here so
-- calibration is a one-line change + refresh.
wg_const AS (
  SELECT
    10   AS wg_min_years,          -- WG_MIN_LISTING_YEARS (owner: hardcoded, no dropdown)
    6    AS fa_min_years,          -- FA_MIN — First Ascent band floor (keeps 2019-21 IPO decay out)
    200  AS min_mcap_cr,           -- audit par.6b
    1.0  AS min_adv_cr,            -- 22-session avg traded value, Cr
    -60  AS dormant_deep_pct,      -- calibrated 2026-08-24 (deep60_oldhigh: 64 Giants / 15 FA)
    365  AS dormant_high_age_days, -- the 3-yr high must be OLD — dormant, not a fresh crash
    1.8  AS dormant_flat_ratio,    -- 3-yr high/low ratio: long-flat-range arm (78 Giants / 12 FA)
...
-- Layer 0.5: age + mcap + dormancy (migration 172/173 columns).
wg_pool AS (
  SELECT s.id AS equity_id,
         EXTRACT(YEAR FROM AGE(CURRENT_DATE, s.listing_date))::int AS listing_age_years,
         s.pct_from_3y_high, s.days_since_3y_high
  FROM km_equity_symbols s
  CROSS JOIN wg_const c
  WHERE s.is_active AND s.exchange = 'NSE'
    AND s.listing_date IS NOT NULL
    AND EXTRACT(YEAR FROM AGE(CURRENT_DATE, s.listing_date)) >= c.fa_min_years
    AND s.mcap_cr >= c.min_mcap_cr
    AND s.pct_from_3y_high IS NOT NULL      -- compute_dormancy.py has run
    AND (   (s.pct_from_3y_high <= c.dormant_deep_pct
...
wg_scored AS (
  SELECT p.equity_id, p.listing_age_years, p.pct_from_3y_high, p.days_since_3y_high,
    ...
    CASE
      ...
      ELSE 'DORMANT'       -- on the watchlist, no accumulation evidence yet
    END AS wg_phase
  FROM wg_pool p
  JOIN wg_metrics m USING (equity_id)
  WHERE m.adv_cr >= (SELECT min_adv_cr FROM wg_const)
),
-- The two age bands. JOIN stock reuses the fully-assembled display row
-- (an ema_20-less stock has no `stock` row and drops out — acceptable).
wg_giants AS (
  SELECT s.*, g.listing_age_years, g.pct_from_3y_high, g.days_since_3y_high,
         g.gl_acc_days, g.wg_phase,
         row_number() OVER (ORDER BY
           CASE g.wg_phase WHEN 'WAKING' THEN 0 WHEN 'STIRRING' THEN 1 ELSE 2 END,
           g.gl_acc_days DESC, s.equity_id) AS rnk
  FROM wg_scored g
  JOIN stock s USING (equity_id)
  WHERE g.listing_age_years >= (SELECT wg_min_years FROM wg_const)
),
wg_ascent AS (
  SELECT s.*, g.listing_age_years, g.pct_from_3y_high, g.days_since_3y_high,
         g.gl_acc_days, g.wg_phase,
...
-- ============================================================================
-- kd_scan_presets rows — SEBI-observational copy (D39): zones/conditions
-- surfaced, no directive verbs, no conviction ratings (confidence tracking
-- gate — audit par.8.3 — comes before any star display).
-- ============================================================================
...
(
    'waking_giants',
    'Waking Giants',
    'Long-listed companies (10y+) in multi-year dormancy where delivery-backed positions are quietly building',
    'Companies listed 10+ years sitting far below a multi-year high that was set long ago, or in a long flat range - with market cap and turnover floors. Phases are observational: STIRRING marks a run of quiet delivery-backed sessions; WAKING adds relative strength rising while price is still flat. A watchlist of conditions, not a recommendation.',
    11, 150, TRUE,
    'stage_analysis', 'Stage Analysis', '#22c55e', 2,
    'NSE_ONLY', 'daily',
    NULL, NULL, NULL, NULL,
    FALSE
),
(
    'first_ascent',
    'First Ascent',
    'Younger listings (6-10y) emerging from a long quiet phase for the first time - no prior peak overhead',
    'Same dormancy-and-accumulation engine as Waking Giants, banded to companies listed 6-10 years. The distinct read: these names have never had a glory run, so a first awakening carries no overhead supply from long-term holders waiting to exit. Observational conditions, not a recommendation.',
    12, 50, TRUE,
    'stage_analysis', 'Stage Analysis', '#22c55e', 2,
    'NSE_ONLY', 'daily',
    NULL, NULL, NULL, NULL,
...
--
-- Then sanity-check the two new presets:
--   SELECT preset_id, count(*), count(*) FILTER (WHERE wg_phase <> 'DORMANT')
--   FROM km_scan_results WHERE preset_id IN ('waking_giants','first_ascent')
--   GROUP BY 1;
-- Expected pool sizes (2026-08-24 calibration): ~142 giants / ~27 ascent.
-- The 6 bundle presets are byte-identical to migration 170 apart from the
-- appended NULL WG columns — no re-parity needed.
-- ============================================================================
