-- ============================================================================
-- km_migration_176_wg_flat_arm_fix.sql
-- ----------------------------------------------------------------------------
-- Waking Giants / First Ascent v3 — fixes found in the owner's live review of
-- migration 175's first refresh. Supersedes 175's WG parts (run INSTEAD of
-- re-running 175; the 175 ALTERs are kept — nothing here re-adds columns).
-- The six bundle presets + flower_pot_burst remain byte-identical to 170.
--
-- WHAT 175's FIRST REFRESH SHOWED (2026-08-24):
--
--   * THE FLAT ARM ADMITTED BLUE CHIPS. Waking Giants' top rows were
--     RELIANCE, KOTAKBANK, TITAN, PIDILITIND; First Ascent filled with
--     2017-listed insurers (SBILIFE, ICICIGI, HDFCLIFE). A mega cap
--     meandering +/-30% for 3 years has range ratio <= 1.8 — but that is
--     maturity, not dormancy. And a normal large-cap session (delivery
--     60%+, small move, ordinary volume) counts as a "GL day", so they
--     also phased as WAKING. FIX: the flat arm now applies only up to
--     flat_max_mcap_cr (5,000 Cr) — "flat" reads as FORGOTTEN for a
--     small/mid cap; for a mega cap it is just being a mega cap. The
--     deep-fall arm is unchanged and has no mcap ceiling (a fallen giant
--     can be any size). The insurer flood was also why SOLARA (STIRRING,
--     12 GL days) missed First Ascent's cap.
--
--   * ADV WAS SINGLE-EXCHANGE. SHIVALIK trades ~0.96 Cr/day on NSE and
--     ~0.09 Cr on BSE — combined 1.05 Cr, but the NSE-only measure failed
--     the 1 Cr floor by 4 lakh. FIX: wg_adv sums the trailing-22-session
--     avg turnover across every exchange row sharing the ISIN.
--
--   * STIRRING NOISE CAP (owner: "waking is the signal — why show all
--     stirring?"): WAKING rows always show; STIRRING is capped to the
--     top stirring_display_cap (10) by GL days per band. Stirring is kept
--     at all because it is the early-warning tier — the "nobody has
--     noticed yet" window that makes this scanner different from the
--     momentum family — but only its strongest evidence surfaces.
--
-- Post-fix expectation: single-digit-to-teens rows per band, led by real
-- fallen names. SHIVALIK/WALCHANNAG enter the POOL but surface only when
-- their quiet-accumulation evidence fires (today both have 0 GL days —
-- correct dormant-watchlist behavior, step-6 material).
--
-- OPERATIONAL NOTES (same as 170/174/175): run as SUPERUSER; brief matview
-- absence between DROP and CREATE; REFRESH km_scan_results then
-- km_scan_exclusion_counts after COMMIT. No script re-run needed — 175's
-- compute_dormancy.py output is reused as-is.
-- ============================================================================
