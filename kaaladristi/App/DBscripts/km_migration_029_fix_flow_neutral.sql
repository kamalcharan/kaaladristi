-- ============================================================
-- Migration 029 · Fix compute_flow_intelligence Neutral zone fallback
--
-- Bug: When magic_rs_zone = 'Neutral' or NULL (and no raw
-- magic_rs/magic_ma), both v_rs_bullish and v_rs_bearish stay
-- FALSE. This causes all high-volume rows to classify as MIXED
-- regardless of price direction — losing signal entirely.
--
-- Fix: When MagicRS provides no directional signal, fall back
-- to RSI-14 as a directional proxy:
--   v_rs_bullish = (rsi_14 > 52) when zone is Neutral/NULL
--   v_rs_bearish = (rsi_14 < 48) when zone is Neutral/NULL
--   Dead zone 48-52: stays MIXED (genuinely no signal)
--
-- The 48/52 thresholds (vs naive 50) add a 2-point dead zone
-- to avoid noise-driven flip-flopping around the midline.
-- ============================================================
