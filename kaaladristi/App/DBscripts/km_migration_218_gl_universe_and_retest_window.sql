-- ============================================================================
-- km_migration_218_gl_universe_and_retest_window.sql
-- Target DB: kaala_dristi_db
--
-- GOLDEN LINE: BOTH EXCHANGES, AND A RETEST YOU CAN ACTUALLY SEE.
--
-- Owner decisions, 2026-09-18:
--   1. Both Golden Line presets cover NSE *and* BSE. They were declared
--      NSE_ONLY and the matview enforced it, which discarded 73% of every
--      retest ever found (since 1 Jul: BSE 83 vs NSE 30).
--   2. gl_retest looks back SEVEN SESSIONS, not one bar.
--   3. A retest row carries how far price has moved since the retest bar's
--      close, so the UI can optionally show only those that have not run away
--      from the line (< 5%). The gate is a UI toggle, NOT a matview filter —
--      membership stays "a retest happened in the window", which is a fact,
--      and the 5% is a preference.
--
-- WHY: measured on the live DB the day this was written, gl_retest returned
-- ZERO rows, and had returned zero on 20 of the previous 40 sessions. NSE-only
-- + single-bar left 0.75 qualifying stocks per session. Both changes together:
-- 31 retest events over 22 distinct stocks in the last 7 sessions
-- (BSE 25 events / 17 stocks, NSE 6 / 5).
--
-- ⚠ THREE THINGS ANY CONSUMER OF THESE ROWS MUST KNOW
--   * gl_event_date is the RETEST bar; trade_date is TODAY. Every other column
--     on the row (close, rvol, pct_from_gl, dot_*) is today's. Reading
--     gl_event_date as the row's date injects a lookahead error in reverse.
--   * gl_days_above is the retest BAR's value — how long the line had been held
--     when it was tested — not a live count as of today. That is the same
--     number the single-bar version returned, so the column's meaning is
--     unchanged; it is now simply anchored to a bar that may not be today.
--     (The stir_days lesson: a count whose anchor is not stated gets read as
--     something else.)
--   * delivery_pct is NULL on EVERY BSE bar (0 of 4,115 populated on
--     2026-09-17, against 3,105 of 3,371 on NSE). With BSE admitted, most
--     retest rows carry no delivery at all. NULL is never zero, and nothing —
--     UI or VaNi — may narrate delivery conviction for a BSE row.
--
-- A stock that retested more than once inside the window is reported ONCE, on
-- its most recent retest (DISTINCT ON ... ORDER BY trade_date DESC): the
-- freshest test of the line is the one that describes where it stands now.
--
-- gl_breakout is unchanged except for the universe — a reclaim is a today
-- event and stays a single-bar arm.
--
-- This migration is a mechanical transformation of migration 205 (the previous
-- full definition of km_scan_results): same 17 arms, same outer projection,
-- plus three columns and the two rewritten Golden Line CTEs. It was generated
-- by script rather than hand-edited, and verified by creating it against a
-- throwaway PostgreSQL 16 cluster before shipping.
--
-- ⚠ Per the convention adopted at migration 205: this file ENDS with the two
-- REFRESH statements as executable SQL. Forgetting them broke eleven scanners
-- at once, twice.
-- ============================================================================

-- The whole recreate is one transaction: the two matviews are dropped and
-- rebuilt together, so a failure anywhere leaves the old pair in place rather
-- than a half-swapped scanner surface. (Cut and restored deliberately — an
-- earlier draft of this file lost the DROPs with the header and would not have
-- run against a database that already had km_scan_results.)
BEGIN;
