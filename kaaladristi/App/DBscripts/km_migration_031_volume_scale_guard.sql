-- ============================================================
-- Migration 031 · Volume Scale Guard for Flow Intelligence
--
-- Data quality issue: km_index_eod has a volume scale
-- discontinuity around 2026-03-25 for NIFTY 50 and possibly
-- other indices. Pre-Mar-25 volume ~500K/day, post-Mar-25
-- ~400M/day. RVOL (50-day average) spans this boundary and
-- drops to near-zero (0.04-0.07) for pre-discontinuity dates.
--
-- This produces false LOW_VOLUME and VACUUM_DOWN signals.
-- TVOL (20-day average) is unaffected — stays within one
-- scale period.
--
-- Two guards added:
--
-- 1. ZERO-VOLUME GUARD: When volume = 0 (holiday/missing),
--    skip computation entirely — leave all flow fields NULL.
--
-- 2. SCALE MISMATCH GUARD: When RVOL < 0.1 AND TVOL > 0.5,
--    the RVOL is unreliable (scale discontinuity). NULL out
--    flow_type, vacuum_flag, volume_divergence_flag.
--    These depend on RVOL. accum_distrib is self-protecting
--    (requires RVOL >= 3.0, so near-zero RVOL never triggers).
-- ============================================================
...
BEGIN
  ...
  FOR i IN start_idx..n LOOP
    ...
    -- accum_distrib is self-protecting (requires RVOL >= 3.0).
