-- ============================================================
-- Migration 030 · Volume Divergence Flag
--
-- Adds volume_divergence_flag to EOD tables and patches
-- compute_flow_intelligence() to compute it.
--
-- Volume divergence = price moving on DECLINING (but real) volume.
-- Distinct from vacuum_flag (price moving on near-ZERO volume).
-- Mutually exclusive by design: avg_rvol > 0.5 guard ensures
-- divergence only fires when real volume exists.
--
-- Values:
--   VOLUME_DIV_UP   — price rising + volume declining (weakening rally)
--   VOLUME_DIV_DOWN — price falling + volume declining (weakening selloff)
--   NULL            — no divergence
--
-- Sustained decline check: rvol[i] < rvol[i-3] AND rvol[i-3] < rvol[i-5]
-- requires two intermediate points declining, preventing single-bar
-- RVOL dips from triggering false positives.
-- ============================================================

-- ── Schema: add column to both EOD tables ──────────────────
ALTER TABLE km_index_eod ADD COLUMN IF NOT EXISTS volume_divergence_flag TEXT;
ALTER TABLE km_equity_eod ADD COLUMN IF NOT EXISTS volume_divergence_flag TEXT;

COMMENT ON COLUMN km_index_eod.volume_divergence_flag IS
  'Volume divergence: VOLUME_DIV_UP | VOLUME_DIV_DOWN | NULL. Price moving on declining (but real) volume.';
COMMENT ON COLUMN km_equity_eod.volume_divergence_flag IS
  'Volume divergence: VOLUME_DIV_UP | VOLUME_DIV_DOWN | NULL. Price moving on declining (but real) volume.';


-- ── Patched compute_flow_intelligence ──────────────────────
