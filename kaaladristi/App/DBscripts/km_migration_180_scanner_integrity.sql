-- =====================================================================
-- km_migration_180_scanner_integrity.sql
-- Target database: kaala_dristi_db
-- Scanner integrity: column contract + universe + liquidity floor
-- POA: docs/claude/scanner-integrity-poa.md
-- =====================================================================
-- Supersedes migration 176. Fixes three defects found by the full
-- 17-preset audit (2026-08-24). Run INSTEAD of re-running 176.
--
-- D1 - COLUMN CONTRACT. The UI renders score_5d, score_22d, rsi_14,
--   avg_amt_5d and supertrend_dir for every matview-backed preset, but
--   km_scan_results has NEVER carried them (verified against migrations
--   147/170/174/175/176 - zero references in all five), so they render
--   as "-" in every row of all six presets. The eight direct-query
--   presets are unaffected because they SELECT from km_equity_eod
--   directly: that is the entire reason Breakout Surge looks healthy
--   while Strength Confluence looks broken. Source columns verified
--   present and 92-100% populated on both exchanges before adding.
--
-- D2 - UNIVERSE VIOLATION. kd_scan_presets.universe is the declared
--   contract and the JS engine enforced it via buildNsePreferredIds().
--   The matview never ported that field: quiet_accumulation is declared
--   NSE_ONLY yet returned 14 BSE rows. (smart_money only looked correct
--   by accident - its symbol ~ '^[A-Z]' gate drops numeric BSE scrip
--   codes as a side effect of an unrelated rule.) Now explicit.
--
-- D3 - NO LIQUIDITY FLOOR ANYWHERE. Measured 2026-08-24, rows under
--   Rs 1 Cr/day turnover: power_sell 24/25, quiet_accumulation 23/25,
--   power_buy 19/25, smart_money 18/25, distribution_warning 10/12 -
--   Strength Confluence rank 3 was a Rs 2.46 stock trading Rs 1 lakh a
--   day. conviction_flow was clean (0/50) precisely because its gate
--   already carries avg_amt_22d > 1.5; that is the control proving the
--   cause. Why it surfaced now: the old JS bundle fetched symbols with
--   .limit(8000) against 10,400 active symbols, silently truncating
--   ~2,400 - skewed to the most recently inserted rows, i.e. exactly
--   the newly-admitted micro-caps. The matview reads uncapped, which
--   correctly removed an accidental shield. Full-universe coverage is a
--   Settled Decision so re-truncating is not an option; an explicit
--   floor is. Owner decision 2026-08-24: uniform Rs 1 Cr, matching the
--   Waking Giants precedent. Sizing (power_buy gate, 25 slots): no
--   floor 294 candidates -> Rs 1 Cr leaves 69. Comfortable headroom.
--
-- D4 - three dead vani_rules (smart_money, quiet_accumulation,
--   distribution_warning produce zero flags) are a SEPARATE defect, NOT
--   a symptom of D3. That hypothesis was tested against live data on
--   2026-08-24 and DISPROVED: the flags do not intersect these presets'
--   gates at all, at any liquidity level.
--     is_vani_smart : 37 stocks -> 14 NSE -> only 4 clear
--                     delivery_pct > 60 -> 0 clear smart_money's full gate
--     is_vani_s2    : 19 stocks -> 0 carry accum_distrib='ACCUMULATION',
--                     which quiet_accumulation requires
--   So the vani_rule assignments are mismatched to what these presets
--   actually select on (is_vani_s2 is a Stage-2 quality overlay; it has
--   no reason to coincide with an ACCUMULATION + rising-sniper gate).
--   Left OPEN deliberately: re-pointing a preset's vani_rule is a
--   product decision about intent, not a bug fix, and silently retuning
--   a threshold to make a chip light up would be worse than the dash.
--   The nightly contract check reports it as a warning until resolved.
--
-- OPERATIONAL (same as 170/174/175/176): run as SUPERUSER; brief
-- matview absence between DROP and CREATE WITH NO DATA; REFRESH
-- km_scan_results then km_scan_exclusion_counts after COMMIT.
-- =====================================================================
...
power_buy AS (
  ...
  WHERE s.industry IS NOT NULL
    AND COALESCE(s.avg_amt_22d, 0) >= 1.0                                   -- D3: liquidity floor (owner: uniform Rs 1 Cr)
...
fpb AS (
  ...
  WHERE (is_burst OR is_shatter OR setup_recent10 = 1)
    AND COALESCE(avg_amt_22d, 0) >= 1.0   -- D3: liquidity floor (applied at final
                                          -- selection so the 140-day compression
                                          -- window above stays complete)
),
...
-- ============================================================================
-- kd_scan_presets — v3 copy (D39-observational).
-- ============================================================================
...
(
    'waking_giants',
    'Waking Giants',
    'Long-listed companies that fell hard from an old peak - shown only when quiet delivery-backed building appears',
    'Companies with 10+ years of listed history (earliest listing across exchanges) that at some point fell at least half from a 3-year high set over a year ago and have not fully recovered - or small/mid caps that sat in a long flat range - with market-cap and combined-exchange turnover floors. A name appears ONLY when evidence shows: Waking = quiet delivery-backed building plus relative strength rising while price is still flat; Stirring (top few only) = the building without the strength turn yet. Observational conditions, not a recommendation.',
    11, 60, TRUE,
...
(
    'first_ascent',
    'First Ascent',
    'Younger listings (6-10y) emerging from a deep quiet phase for the first time - shown only on accumulation evidence',
    'Same dormancy-and-accumulation engine as Waking Giants, banded to companies listed 6-10 years. The distinct read: these names have never had a glory run, so a first awakening carries no overhead supply from long-term holders waiting to exit. Appears only on Waking / top-Stirring evidence. Observational conditions, not a recommendation.',
    12, 30, TRUE,
    'discovery', 'Discovery', '#14b8a6', 5,
    'NSE_ONLY', 'daily',
    NULL, NULL, NULL, NULL,
