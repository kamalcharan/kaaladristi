-- ============================================================================
-- Migration 181 — remove the Rs 1 Cr liquidity floor from km_scan_results
-- ============================================================================
-- Migration 180 added two things to this matview. One was a fix: the columns
-- the UI renders (score_5d, score_22d, rsi_14, avg_amt_5d, supertrend_dir)
-- were absent, so every row showed a dash. That stays.
--
-- The other was a uniform Rs 1 Cr/day turnover floor, added as hygiene without
-- first measuring what it removed. Measured afterwards on 2026-08-24 data, the
-- same floor on the direct scanners cut Stage 2 Watch from 447 rows to 148
-- (-67%) and Stage 2 Leaders from 1,023 to 414 (-60%). Inside this matview it
-- left smart_money with 7 rows, quiet_accumulation with 8 and
-- distribution_warning with 2 — result sets too small for those scanners to
-- mean anything, and too small to even choose a VaNi rule against.
--
-- Removing two thirds of a scanner's universe is a product decision. It gets
-- made deliberately, per scanner, with the before/after count recorded — not
-- slipped in as a hygiene gate. So it comes out of all six places migration
-- 180 put it (5 preset arms + the flower_pot_burst final selection).
--
-- KEPT from migration 180, deliberately:
--   * the five UI columns          — that fixed a reported bug
--   * the NSE-only enforcement     — quiet_accumulation is declared NSE_ONLY
--                                    in kd_scan_presets and was returning 14
--                                    BSE rows; that is a contract violation,
--                                    not a judgement call
--
-- Row counts WILL rise on every preset after this. That is the intent.
--
-- After running:  REFRESH MATERIALIZED VIEW km_scan_results;
--                 REFRESH MATERIALIZED VIEW km_scan_exclusion_counts;
--                 python scripts/audit_scanner_contract.py
-- ============================================================================
