...
def intent_facts(ctx, intent):
    ...
    for r in rows[:5]:
        ...
        if c.get('leaders_pct') is None: missing.append('Leader share unavailable because the sample is insufficient')
        elif c['leaders_pct'] < 60: missing.append('measured Leader share is below 60%; this is not missing data')
