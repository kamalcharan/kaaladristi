...
INTENTS: dict[str, VaNiIntent] = {
    ...
    "scanner.why_highlighted_gl": VaNiIntent(
        ...
        system_prompt=(
            ...
            "are on the list, their average distance above the line, for a "
            "retest their average sessions already held above the line, "
            "their average volume (RVOL), and up to 2 named examples with "
            "their own numbers."
            "\n\n"
            "TWO THINGS ABOUT THIS LIST YOU MUST NOT GET WRONG (migration "
            "218). First, a RETEST list covers the LAST SEVEN TRADING "
            "SESSIONS, not one day: never say these stocks printed the event "
            "'today' unless you are given a figure that says so. Say 'on this "
            "list' or 'in the last seven sessions'. A BREAKOUT list is still "
            "a single session. Second, this screener covers NSE *and* BSE, "
            "and delivery figures are NOT PUBLISHED FOR BSE at all — so "
            "never describe delivery, delivery percentage or delivery "
            "conviction for any named stock here, in either direction. A "
            "missing delivery figure means it was not measured on that "
            "exchange; it never means delivery was low."
            "\n\n"
            ...
            "the count is zero, say plainly that no stock on this list "
            "printed the event over the period covered."
            + _VANI_RULES.replace(
                "No bullet points — write flowing paragraphs. About 150 words.",
                "Short bullet points are REQUIRED here (see the format "
                "instructions above) — this overrides the no-bullets house "
    ...
    "equity.why_in_context": VaNiIntent(
        ...
        system_prompt=(
            ...
            "\n\n"
            "If page_context begins with 'Sector Rotation /', the stock is a recorded constituent of the named sector. Explain its own supplied signals and how they relate to the sector question; do not invent a sector-membership threshold or imply that sector strength proves stock strength. Otherwise, "
