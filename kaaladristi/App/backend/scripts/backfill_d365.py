...
def _sql_d365_for_date(conn, target_date: str) -> int:
    ...
    For each equity row on target_date, finds the most recent trading day
    between 335 and 395 days ago (365 ± 30 day tolerance), matching the
    original bisect_left logic exactly. PostgreSQL executes this as a single
    UPDATE — no rows are fetched into Python memory.
    """
    sql = """
        UPDATE km_equity_eod AS e
