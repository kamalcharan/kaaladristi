"""
generate_saturn_windows.py

Generate Saturn rule transit windows from km_planetary_positions
and insert into km_rule_transits (ON CONFLICT DO NOTHING).

Rules generated:
  1. TR-SAT-RET        — Plain Saturn retrograde windows (motion almanac, migration 130)
  2. TRN-SAT-MAN-TRN   — Saturn sign transit windows (Journey of Saturn, migration 130)
...
Run:
  ...
  DB_PRIMARY=postgresql://user:pass@host:5432/kaala_dristi_db python3 generate_saturn_windows.py
...
# ── Rule 1: Plain Saturn Retrograde (motion almanac) ──────────────────────────

def generate_plain_retrograde(cur, rule_id: int) -> tuple[int, int]:
    windows = detect_islands(cur, "Saturn", "retrograde")
    rows = []
    for w in windows:
        snap = json.dumps({
...
# ── Rule 2: Saturn Sign Transits (Journey of Saturn) ──────────────────────────
    ...
    try:
        with conn:
            with conn.cursor() as cur:
                ...
                all_rule_codes = ["TR-SAT-RET", "TRN-SAT-MAN-TRN"]
                ...
                if "TR-SAT-RET" in rule_ids:
                    summary["TR-SAT-RET"] = generate_plain_retrograde(
                        cur, rule_ids["TR-SAT-RET"]
                    )

                if "TRN-SAT-MAN-TRN" in rule_ids:
                    summary["TRN-SAT-MAN-TRN"] = generate_sign_transits(
                        cur, rule_ids["TRN-SAT-MAN-TRN"]
                    )
