"""
generate_jupiter_windows.py

Generate Jupiter rule transit windows from km_planetary_positions
and insert into km_rule_transits (ON CONFLICT DO NOTHING).

Rules generated:
  1. TR-JUP-RET        — Plain Jupiter retrograde windows (motion almanac, migration 129)
  2. TRN-JUP-MAN-TRN   — Jupiter sign transit windows (Journey of Jupiter, migration 129)
...
Run:
  ...
  DB_PRIMARY=postgresql://user:pass@host:5432/kaala_dristi_db python3 generate_jupiter_windows.py
...
# ── Rule 1: Plain Jupiter Retrograde (motion almanac) ─────────────────────────

def generate_plain_retrograde(cur, rule_id: int) -> tuple[int, int]:
    windows = detect_islands(cur, "Jupiter", "retrograde")
    rows = []
    for w in windows:
        snap = json.dumps({
            "event": "jupiter_retrograde",
...
# ── Rule 2: Jupiter Sign Transits (Journey of Jupiter) ────────────────────────
    ...
    try:
        with conn:
            with conn.cursor() as cur:
                ...
                all_rule_codes = ["TR-JUP-RET", "TRN-JUP-MAN-TRN"]
                ...
                if "TR-JUP-RET" in rule_ids:
                    summary["TR-JUP-RET"] = generate_plain_retrograde(
                        cur, rule_ids["TR-JUP-RET"]
                    )

                if "TRN-JUP-MAN-TRN" in rule_ids:
                    summary["TRN-JUP-MAN-TRN"] = generate_sign_transits(
                        cur, rule_ids["TRN-JUP-MAN-TRN"]
                    )
