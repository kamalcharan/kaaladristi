/**
 * Weekly Decliners — adapter for the Scanner Story Page.
 *
 * The mirror of Weekly Movers: this preset selects on close BELOW the previous
 * week's close — week-to-date weakness, not a breakdown through a floor. The
 * same honesty applies in reverse. It is a wide, low-bar filter (1,340 NSE rows
...
const SETUP_KEY = 'weekly_decliners';
const SETUP_LABEL = 'Weekly Decliners';
const PERIOD_WORD = 'week';
const PRIOR_LOW_BARS = 1;
const ZONE_WIDTH = 0.015;

export const weeklyDeclinersAdapter: SetupAdapter = (weekly, latest, identity) => {
  const ema50w = smaFromEnd(weekly, 50, (b) => b.close);
  const refClose = latest.prev_week_close;
