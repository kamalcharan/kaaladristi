...
import { api, guestApi } from '@/services/apiClient';
...
/** Landing-page (logged-out) twin of fetchPanchang — same shape, guest client. */
export async function fetchGuestPanchang(date: string): Promise<DailyPanchang | null> {
  const res = await guestApi.fetch(`/api/guest/panchang/daily?date=${encodeURIComponent(date)}`);
  if (res.status === 404) return null;
  if (!res.ok) throw new Error(`[panchang] HTTP ${res.status}`);
  return res.json() as Promise<DailyPanchang>;
}
...
export async function fetchConfluenceHeatmap(date: string): Promise<ConfluenceHeatmap> {
  const res = await api.fetch(`/api/confluence/heatmap?date=${encodeURIComponent(date)}`);
...
export async function fetchConfluenceHistorical(): Promise<ConfluenceData> {
  const res = await api.fetch('/api/confluence/historical');
...
export async function fetchConfluenceTimeline(days: number): Promise<ConfluenceTimelineEntry[]> {
  const res = await api.fetch(`/api/confluence/timeline?days=${days}`);
