...
function SingleToast({ item, onDismiss }: { item: ToastItem; onDismiss: (id: string) => void }) {
  ...
  return (
    <div
      className={cn(
        'flex items-start gap-3 px-4 py-3.5 rounded-xl border border-kd-border border-l-4 shadow-xl shadow-black/40 min-w-[280px] max-w-sm backdrop-blur-xl',
        bg,
        border,
      )}
    ...
    >
      ...
      <p className="text-sm text-[var(--text-primary)] flex-1 leading-snug">{item.message}</p>
      <button
