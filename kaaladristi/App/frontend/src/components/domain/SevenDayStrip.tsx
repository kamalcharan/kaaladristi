...
function dayLabel(iso: string) {
  ...
  return { dow: DOW_ABBR[dt.getUTCDay()], dd: String(dt.getUTCDate()) };
}
        ...
        {active.length === 0 ? (
          ...
          active.map(inf => {
            ...
            return (
              <span
                key={inf.id}
                className={cn('w-2 h-2 rounded-full cursor-default', dotClass(inf.market_impact))}
                title={`${inf.astro_event}${opt ? ` · ${opt.label}` : ''}`}
              />
            );
          })
        )}
