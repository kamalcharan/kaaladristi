import {BriefcaseBusiness,Star,Link2} from 'lucide-react';
...
export default function SectorPersonalConnections({sectors,date,sourceKey,membership}:{sectors:PersonalSector[];date?:string;sourceKey?:string;membership?:SectorMembership}) {
 ...
 if(authLoading) body=<p role="status">Loading your account…</p>;
  ...
  const cards=matches.map(({stock,kind,sectors:connected})=><article key={stock.equity_id} className="vani-personal-card" data-kind={kind.toLowerCase()}>
   <p className="vani-personal-heading">{kind==='Position'?<BriefcaseBusiness size={16} aria-hidden="true"/>:<Star size={16} aria-hidden="true"/>}<strong>{kind}</strong> · <Link className="underline break-words" to={`/chart/equity/${stock.equity_id}?name=${encodeURIComponent(stock.symbol)}`}>{stock.symbol}</Link></p>
   {connected.map(s=><p key={s.id} className="vani-sector-connection"><Link className="underline break-words" to={`/sector-rotation/${s.id}${date?`?asof=${date}`:''}`}>{s.name}</Link> · {s.reading}</p>)}
  </article>);
  body=matches.length?<><p>{matches.length} of your saved stocks {matches.length===1?'is':'are'} linked to sectors in this reading.</p>{cards.slice(0,3)}{cards.length>3&&<details data-vani-detail="connections"><summary className="sector-question cursor-pointer">Show {cards.length-3} more connections</summary><div className="space-y-2 pt-2">{cards.slice(3)}</div></details>}<p className="text-xs">A sector’s condition does not establish the stock’s own strength. Open its chart to inspect its participation.</p><Link className="underline" to="/bookmarks">Manage bookmarks and positions</Link></>:<><p>None of your saved stocks are linked to the sectors highlighted in this reading.</p><Link className="underline" to="/bookmarks">View bookmarks and positions</Link></>;
 }
 return <section className="vani-personal space-y-2 border-t border-[var(--border)] pt-3 text-sm" aria-label="Connected to your stocks"><h3 className="vani-personal-heading"><Link2 size={17} aria-hidden="true"/>Connected to your stocks</h3><div className="vani-personal-content" data-state={query.error?'error':query.data&&personalConnections(query.data,sectors).length?'matched':'empty'}>{body}</div><p className="text-xs text-muted">Your saved stocks reflect your account today. Connections use recorded basket membership, including when viewing an earlier market session.</p></section>;
}
