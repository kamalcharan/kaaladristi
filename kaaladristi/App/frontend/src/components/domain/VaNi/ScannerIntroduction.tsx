...
export default function ScannerIntroduction({presetId,activeQuestion=false}:{presetId:string;activeQuestion?:boolean}) {
 ...
 <section><h3 className="text-sm font-semibold mb-2">What this scanner finds</h3><p className="text-sm leading-6">{copy.story}</p></section>
 ...
 <details className="mt-3"><summary>Understand the signals</summary><div className="vani-evidence-sections">{copy.signals.map(signal=><section key={signal.title}><h4><mark className="vani-evidence-value">{signal.title}</mark></h4><p className="text-sm leading-6">{signal.text}</p></section>)}</div></details>
 </details>
}
