...
<div className="vani-followups" onClick={revealVaniReading}><p>Continue your research</p>    <div className="flex flex-col gap-2" aria-label="Continue sector research">
      {(Object.entries(questions) as [Question,string][]).filter(([id])=>id!==question).map(([id,label]) => <button key={id} className="sector-question text-left" aria-pressed={question === id} onClick={() => { setQuestion(id); trackVani('intent_selected', {page:'sector_detail',mode:'longer_term',intent_id:`sector.detail.${id}`,months:c.months ?? 6,source:'manual'}); }}>{label}</button>)}
    </div>
</div>
  </aside>;
}
