import {DISCOVERY_SCANNER_IDS,FLOW_SCANNER_IDS,STAGE_SCANNER_IDS,SCANNER_INTRODUCTIONS} from '@/constants/scannerIntroductions'
...
export default function ScannerCompanionDock({presetId}:{presetId:string}) {
 ...
 return <div id="scanner-vani-host" className="scanner-vani-host">
  {(isFlow||isStage||isDiscovery)&&<ScannerCompanionShell key={presetId} portal={false} presetId={presetId} subtitle={`${SCANNER_INTRODUCTIONS[presetId].name} · ${isFlow?'Flow':isStage?'Stage':'Discovery'} research`}>{null}</ScannerCompanionShell>}
 </div>
}
