  ...
  return <aside {...analytics} aria-label="VaNi Market Structure companion" className="ph-no-capture w-full lg:w-[var(--vani-w)] shrink-0 self-start lg:sticky rounded-2xl border border-[var(--border)] bg-[var(--card)] overflow-hidden" style={{ top: 'calc(var(--topbar-h) + 1rem)' }}>
    ...
    <div className={`p-4 space-y-4 lg:max-h-[calc(100vh-220px)] overflow-y-auto ${mobileExpanded ? 'block' : 'hidden'} lg:block`}>
      ...
      {ready && <details data-vani-detail="evidence" className="border border-[var(--border)] rounded-xl p-3"><summary className="text-xs cursor-pointer">Show the evidence</summary>
        {target === 'momentum' ? <div className="h-40 mt-3" role="img" aria-label="ROC 13, ROC 55 and the five-session signal over the selected period"><ResponsiveContainer width="100%" height="100%"><LineChart data={data.roc}><XAxis dataKey="trade_date" hide /><YAxis width={40} tick={{ fontSize: 9, fill: 'var(--text-muted)' }} /><Tooltip contentStyle={{ background: 'var(--card)', borderColor: 'var(--border)' }} /><ReferenceLine y={0} stroke="var(--text-muted)" /><Line name="ROC 13" dataKey="roc_13" stroke="var(--accent)" dot={false} /><Line name="ROC 55" dataKey="roc_55" stroke="var(--text-muted)" strokeDasharray="4 3" dot={false} /><Line name="Signal (5)" dataKey="sma_breadth" stroke="var(--text-primary)" dot={false} /></LineChart></ResponsiveContainer></div> : <dl className="text-xs mt-3 space-y-2">{(['pct_above_20', 'pct_above_50', 'pct_above_150'] as const).map((k, i) => <div key={k} className="flex justify-between"><dt>Above {[20, 50, 150][i]} EMA</dt><dd>{data.breadth.at(-1)?.[k]?.toFixed(1) ?? '—'}%</dd></div>)}<div className="flex justify-between"><dt>Breadth score</dt><dd>{data.breadth.at(-1)?.breadth_score?.toFixed(1) ?? '—'}</dd></div><div className="flex justify-between"><dt>Daily pressure</dt><dd>U{data.breadth.at(-1)?.up_5pct ?? '—'} · D{data.breadth.at(-1)?.down_5pct ?? '—'}</dd></div><div className="flex justify-between"><dt>Five-day extremes</dt><dd>U{data.breadth.at(-1)?.up_20pct_5d ?? '—'} · D{data.breadth.at(-1)?.down_20pct_5d ?? '—'}</dd></div></dl>}
