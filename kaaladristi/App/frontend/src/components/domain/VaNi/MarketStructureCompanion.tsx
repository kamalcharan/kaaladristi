...
const lesson = [
  ...
  { title: 'Keep interpretation separate', text: 'A filled ball marks a confirmed zone or momentum event. Fear and Greed remain framework labels, and no colour or event establishes a reversal or trade instruction.', section: 'framework' as const },
];
...
export default function MarketStructureCompanion() {
  ...
  const depth: ExplanationDepth = 'simple';
  ...
  const choose = (id: StructureIntentId) => { setPresentationCycle(n => n + 1); setIntent(id); trackVani('intent_selected', {...analyticsContext,intent_id:id,source:'manual'}); };
  ...
  return <aside {...analytics} aria-label="VaNi Market Structure companion" className="ph-no-capture w-full lg:w-[var(--vani-w)] shrink-0 self-start lg:sticky rounded-2xl border border-[var(--border)] bg-[var(--card)] overflow-hidden" style={{ top: 'calc(var(--topbar-h) + 1rem)' }}>
    <div className="p-4 border-b border-[var(--border)]"><VaNiBrand subtitle={<>Market Structure · All NSE · {data.period}-session window</>} />
      <p className="text-[12px] text-muted mt-1">Breadth: {data.breadthDate ?? 'unavailable'} · ROC: {data.rocDate ?? 'unavailable'}</p>
      <p className="text-[12px] text-muted mt-1">Dates identify the source market sessions. Readings use closing data, not live prices.</p>
    ...
    <div className={`p-4 space-y-4 lg:max-h-[calc(100vh-220px)] overflow-y-auto ${mobileExpanded ? 'block' : 'hidden'} lg:block`}>
      ...
      <div><p className="text-xs font-medium text-[var(--text-primary)] mb-2">{definition.label}</p>
        {data.isLoading && isLive ? loader : !ready && isLive ? <p role="status" className="text-xs text-muted">Market readings are unavailable. The learning intents below remain available.</p> : consulting ? loader : issue ? <div role="status"><p className="text-xs text-muted">{issue}</p><button className="text-xs text-accent-indigo mt-2" onClick={retryReading}>Try again</button></div> : <p className="text-sm leading-7 whitespace-pre-wrap text-[var(--text-secondary)]">{reading.data?.response}</p>}
      ...
      {ready && <details data-vani-detail="evidence" className="border border-[var(--border)] rounded-xl p-3"><summary className="text-xs cursor-pointer">Show the evidence</summary>
        {target === 'momentum' ? <div className="h-40 mt-3" role="img" aria-label="ROC 13, ROC 55 and the five-session signal over the selected period"><ResponsiveContainer width="100%" height="100%"><LineChart data={data.roc}><XAxis dataKey="trade_date" hide /><YAxis width={40} tick={{ fontSize: 11, fill: 'var(--text-muted)' }} /><Tooltip contentStyle={{ background: 'var(--card)', borderColor: 'var(--border)' }} /><ReferenceLine y={0} stroke="var(--text-muted)" /><Line name="ROC 13" dataKey="roc_13" stroke="var(--accent)" dot={false} /><Line name="ROC 55" dataKey="roc_55" stroke="var(--text-muted)" strokeDasharray="4 3" dot={false} /><Line name="Signal (5)" dataKey="sma_breadth" stroke="var(--text-primary)" dot={false} /></LineChart></ResponsiveContainer></div> : <dl className="text-xs mt-3 space-y-2">{(['pct_above_20', 'pct_above_50', 'pct_above_150'] as const).map((k, i) => <div key={k} className="flex justify-between"><dt>Above {[20, 50, 150][i]} EMA</dt><dd>{data.breadth.at(-1)?.[k]?.toFixed(1) ?? '—'}%</dd></div>)}<div className="flex justify-between"><dt>Breadth score</dt><dd>{data.breadth.at(-1)?.breadth_score?.toFixed(1) ?? '—'}</dd></div><div className="flex justify-between"><dt>Daily pressure</dt><dd>U{data.breadth.at(-1)?.up_5pct ?? '—'} · D{data.breadth.at(-1)?.down_5pct ?? '—'}</dd></div><div className="flex justify-between"><dt>Five-day extremes</dt><dd>U{data.breadth.at(-1)?.up_20pct_5d ?? '—'} · D{data.breadth.at(-1)?.down_20pct_5d ?? '—'}</dd></div></dl>}
        {reading.data?.facts?.map((fact, i) => <p key={i} className="text-[12px] text-muted mt-2">{fact}</p>)}
        <button className="text-xs text-accent-indigo mt-3" onClick={() => showStructureEvidence(target)}>Show me on the page →</button>
      </details>}
      <div onClick={revealVaniReading}><p className="text-[12px] text-muted uppercase tracking-wide mb-2">Continue your research</p>{STRUCTURE_FOLLOWUPS[intent].filter(id => ready || STRUCTURE_INTENTS[id].kind === 'static').map(id => <button key={id} className="text-left w-full p-3 mb-2 rounded-xl border border-[var(--border)] text-xs text-[var(--text-secondary)] hover:border-accent-indigo" onClick={() => choose(id)}>{STRUCTURE_INTENTS[id].label} →</button>)}{!ready && <button className="text-xs text-accent-indigo" onClick={() => choose('structure.ema')}>Understand participation</button>}</div>
      ...
      <p className="text-[12px] text-muted">Research and education. No buy/sell recommendations.</p>
      </>}
    </div>
  </aside>;
}
