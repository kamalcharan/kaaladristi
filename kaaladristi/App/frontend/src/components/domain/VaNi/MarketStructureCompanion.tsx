...
export default function MarketStructureCompanion() {
  ...
  const choose = (id: StructureIntentId) => { setIntent(id); setDepth('brief'); trackEvent('vani_intent_selected', { intent_id: id, page: 'market_structure', period: data.period }); };
  ...
  return <aside aria-label="VaNi Market Structure companion" className="w-full lg:w-[var(--vani-w)] shrink-0 self-start lg:sticky rounded-2xl border border-[var(--border)] bg-[var(--card)] overflow-hidden" style={{ top: 'calc(var(--topbar-h) + 1rem)' }}>
    <div className="p-4 border-b border-[var(--border)]"><h2 className="text-lg font-serif text-[var(--text-primary)]">VaNi · वाणी</h2>
      ...
      <button className="lg:hidden text-sm text-accent-indigo mt-3" aria-expanded={mobileExpanded} onClick={() => setMobileExpanded(!mobileExpanded)}>{mobileExpanded ? 'Close explanation' : 'Help me read this page'}</button>
    </div>
    <div className={`p-4 space-y-4 lg:max-h-[calc(100vh-220px)] overflow-y-auto ${mobileExpanded ? 'block' : 'hidden'} lg:block`}>
      ...
      <div><p className="text-xs font-medium text-[var(--text-primary)] mb-2">{definition.label}</p>
        ...
        {data.isLoading && isLive ? <p role="status" className="text-xs text-muted">Loading the market readings…</p> : !ready && isLive ? <p role="status" className="text-xs text-muted">Market readings are unavailable. The educational questions below remain available.</p> : (reading.isFetching || reading.data?.pending) ? <p role="status" className="text-xs text-muted">Preparing this explanation…</p> : issue ? <div role="status"><p className="text-xs text-muted">{issue}</p><button className="text-xs text-accent-indigo mt-2" onClick={retryReading}>Try again</button></div> : <p className="text-sm leading-7 whitespace-pre-wrap text-[var(--text-secondary)]">{reading.data?.response}</p>}
        {!reading.isFetching && reading.data?.log_id && <VaNiFeedback key={reading.data.log_id} logId={reading.data.log_id} />}
      </div>
      {ready && <details className="border border-[var(--border)] rounded-xl p-3"><summary className="text-xs cursor-pointer">Show the evidence</summary>
