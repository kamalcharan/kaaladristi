      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0 8px 8px' }}>
        ...
        {data.length === 0 && (
          <div style={{ textAlign: 'center', padding: '12px 0', fontSize: 11,
            color: 'var(--text-faint)' }}>no matches today</div>
        )}
      </div>
    </div>
  )
    ...
    <div style={{ flex: 1, display: 'flex', flexDirection: 'column',
      ...
      <div>
        <div style={{ fontSize: 9, textTransform: 'uppercase', letterSpacing: '0.06em',
          color: 'var(--text-muted)', marginBottom: 3 }}>Next occurrence</div>
        {nextSignal
          ? (
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
...
function ReturnBar({ value }: { value: number | null }) {
  if (value == null) return <span style={{ color: 'var(--text-faint)', fontSize: 9 }}>—</span>
...
function InstanceRow({ inst }: { inst: CorrelationInstance }) {
  ...
  return (
    <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '4px 0',
      ...
      <span style={{ color: 'var(--text-muted)', flexShrink: 0, width: 32 }}>{inst.duration_days}d</span>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap' }}>
        {[itemA, itemB].map((id, i) => (
          <React.Fragment key={id}>
            ...
            {i === 0 && <span style={{ fontSize: 9, color: 'var(--text-muted)' }}>∩</span>}
          </React.Fragment>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', gap: 8, paddingTop: 4, borderTop: '1px solid color-mix(in srgb, var(--text-primary) 6%, transparent)' }}>
        <button
          ...
          style={{ flex: 1, padding: '5px 0', fontSize: 9, borderRadius: 5,
            ...
            color: 'var(--text-muted)', cursor: 'not-allowed', fontFamily: 'var(--font-mono,monospace)' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '10px 12px 8px',
        ...
        {editMode && (
          <div
            ...
            style={{ cursor: 'grab', color: 'var(--text-faint)', fontSize: 14,
        ...
        <button
          ...
          style={{ width: 22, height: 22, borderRadius: 5, border: 'none',
            ...
            color: isMaximized ? 'var(--accent)' : 'var(--text-muted)',
          ...
          onMouseLeave={e => {
            ...
            ;(e.currentTarget).style.color = isMaximized ? 'var(--accent)' : 'var(--text-muted)'
          }}>
          {isMaximized ? '⊟' : '⊞'}
        </button>
