...
export default function DataTab({ equityId, symbol }: { equityId: number; symbol: string }) {
  ...
  return (
    <div className="flex flex-col gap-3">
      ...
      <div className="flex items-center gap-2 flex-wrap">
        {RANGES.map((r) => (
          <button
            ...
            className={cn(
              'px-2 py-1 rounded text-[12px] font-mono border transition-colors',
              range === r
        ...
        <div className="ml-auto flex items-center gap-2">
          <button
            ...
            className="inline-flex items-center gap-1.5 px-2 py-1 rounded border border-kd-border text-[12px] font-mono text-muted hover:text-[var(--text-primary)]"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-1.5 px-2 py-1 rounded border border-kd-border text-[12px] font-mono text-muted hover:text-[var(--text-primary)] disabled:opacity-40"
          >
        ...
        <div className="rounded-lg border border-kd-border bg-kd-card p-3 max-h-80 overflow-y-auto">
          <div className="flex items-center gap-2 mb-2">
            <button onClick={() => setCols(DATA_DEFAULT_COLUMNS)}
              className="text-[12px] font-mono text-accent-indigo hover:underline">Reset to default</button>
          </div>
          {DATA_COLUMN_GROUPS.map((g) => (
            <div key={g.id} className="mb-3">
              <div className="text-[12px] font-mono uppercase tracking-wider text-[var(--text-faint)] mb-1">
                {g.title}
              </div>
              {g.note && (
                <p className="text-[11px] font-mono text-muted mb-1 leading-relaxed">{g.note}</p>
              )}
                ...
                {g.columns.map((c) => {
                  ...
                  return (
                    <button
                      ...
                      className={cn(
                        'px-1.5 py-0.5 rounded text-[12px] font-mono border transition-colors',
      ...
      {isError && (
        <p className="text-[12px] font-mono text-risk-red">
        ...
        <table className="w-full text-[12px] font-mono">
          <thead>
            <tr className="border-b border-kd-border bg-kd-elevated">
              {cols.map((c) => (
                <th key={c} className="px-2 py-1.5 text-right whitespace-nowrap text-[12px] uppercase tracking-wider text-muted">
                  {label(c)}
                </th>
              ))}
            </tr>
