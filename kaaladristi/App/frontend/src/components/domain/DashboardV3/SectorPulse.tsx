...
function PulseRow({ row, onClick }: { row: SectorPulseRow; onClick: () => void }) {
  ...
  return (
    ...
    >
      ...
      <span style={{ ...MONO, fontSize: 12, color: 'var(--text-faint)', flexShrink: 0, width: 28, textAlign: 'right' }}>
    ...
    <div aria-label="Sector flow snapshot">
      <div style={{ display: 'flex', alignItems: 'baseline', gap: 10, marginBottom: 12, flexWrap: 'wrap' }}>
        ...
        {!embedded && <button
          ...
          style={{
            ...MONO, marginLeft: 'auto', fontSize: 12, color: 'var(--gold)',
      ...
      {isLoading ? (
        <div style={{ ...MONO, fontSize: 12, color: 'var(--text-faint)', padding: '12px 0' }}>Loading sector pulse…</div>
      ) : (
        <>
          <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
            {(Object.keys(BUCKET_META) as Bucket[]).map((bucket) => <span key={bucket} style={{
              ...
              color: BUCKET_META[bucket].color, fontSize: 11, textTransform: 'uppercase', letterSpacing: '.06em',
            ...
            return (
              ...
              >
                <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 6 }}>
                  ...
                  <span role="heading" aria-level={4} style={{ ...MONO, fontSize: 12, fontWeight: 700, letterSpacing: '.07em', textTransform: 'uppercase', color: meta.color }}>
                    {meta.title}
                  </span>
                ...
                {rows.length === 0 ? (
                  <div style={{ ...MONO, fontSize: 12, color: 'var(--text-faint)', padding: '6px 6px 8px' }}>none today</div>
                ) : (
                  <>
                    <div className={bucket === 'entering' ? 'sector-pulse-entering-grid' : undefined} style={{ display: 'grid', gridTemplateColumns: bucket === 'entering' ? 'repeat(2, minmax(0, 1fr))' : '1fr', gap: bucket === 'entering' ? 18 : 0 }}>
                      {columns.map((column, columnIndex) => <div key={columnIndex}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '0 6px 4px', borderBottom: '1px solid var(--border)', marginBottom: 2 }}>
                          <span style={{ ...MONO, fontSize: 11, letterSpacing: '.07em', textTransform: 'uppercase', color: 'var(--text-faint)', flex: 1 }}>Sector</span>
                          <span style={{ ...MONO, fontSize: 11, letterSpacing: '.07em', textTransform: 'uppercase', color: 'var(--text-faint)', width: 28, textAlign: 'right', flexShrink: 0 }}>5D</span>
                          <span style={{ ...MONO, fontSize: 11, letterSpacing: '.07em', textTransform: 'uppercase', color: 'var(--text-faint)', width: 28, textAlign: 'right', flexShrink: 0 }}>22D</span>
                          <span style={{ ...MONO, fontSize: 11, letterSpacing: '.07em', textTransform: 'uppercase', color: 'var(--text-faint)', width: 72, flexShrink: 0 }}>Trend</span>
                        </div>
                    ...
                    {overflow > 0 && (
                      <button
                        ...
                        style={{ ...MONO, fontSize: 12, color: 'var(--text-muted)', background: 'none', border: 'none', cursor: 'pointer', padding: '7px 6px 2px' }}
