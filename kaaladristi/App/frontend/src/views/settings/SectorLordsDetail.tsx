...
interface SectorLordsDetailProps {
  onBack: () => void;
}

export default function SectorLordsDetail({ onBack }: SectorLordsDetailProps) {
  ...
  return (
    <div>
      ...
      <div className="flex items-center gap-4 mb-6">
        <button
          onClick={onBack}
          className="w-9 h-9 rounded-xl flex items-center justify-center bg-kd-elevated border border-kd-border text-secondary hover:text-[var(--text-primary)] hover:border-accent-indigo/40 transition-all"
        >
      ...
      {isLoading ? (
        ...
        <>
          ...
          {/* Table */}
          <div className="bg-kd-bg border border-kd-border rounded-2xl overflow-hidden">
                ...
                rows.map((r, i) => (
                  <div
                    ...
                    className={`grid grid-cols-[1fr_auto] gap-4 px-5 py-3 text-sm ${
                      i % 2 === 0 ? 'bg-kd-bg' : 'bg-kd-elevated/15'
                    } hover:bg-kd-elevated/30 transition-colors`}
                  >
