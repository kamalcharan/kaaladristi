...
The recommendations include the four core scanners and the category starting points. Visible styled category radio buttons replace the scanner sidebar. Sibling tabs remain in stable order, with a visible count and recommendation badges. Explicit scanner URLs are preserved. If a configured category default is unavailable, use that category's existing default.

Price Action VaNi uses the persistent left-side companion arrangement shared with sector research; it stacks above the results on smaller screens. Stock-row questions reuse the companion space. There is no separate Ask/Pin/Close drawer flow. Existing intent payloads, filters, evidence and feedback remain unchanged. Flower Pot Burst retains its own API and answer format.
