...
Content is maintained in src/constants/scannerIntroductions.ts, version 3, and shipped with the frontend bundle. There is no time-based expiry, generated-text cache or recurring LLM request. Increment the content version when the copy or underlying rules change. This educational text has no session-specific stocks or counts.
