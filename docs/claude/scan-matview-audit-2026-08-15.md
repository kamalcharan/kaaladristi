...
1. ~~**Migration 170**~~ **WRITTEN 2026-08-16** —
   `km_migration_170_scan_matview_refresh_logic.sql` covers F3, F4, F5, F6.
   Materialized views cannot be ALTERed, so it DROPs and re-CREATEs both views
   from migration 147's definition with those four changes, and re-applies
   migration 151's MAINTAIN grants + 157's `kd_app` ownership (a DROP/CREATE
   resets both; without them the nightly `scan_refresh` fails with "permission
   denied"). Generated by targeted exact-string replacement on 147 rather than
   retyping 775 lines of parity-critical SQL.

   **Pre-verified against live 2026-08-14 data** (read-only, before the owner
   runs anything):
   - Old smart_money gate `pct_accumulation > 60` matches **0 industries** —
     the 0-row bug reproduced exactly. New gate qualifies 13 (decile size 13).
   - The new gate's full preset SQL returns the **same 25 stocks** as the live
     scanEngine, with ranks 8/9/10 (KODYTECH, GML, DELAPLEX) identical; ranks
     1–7 are a true 7-way tie at `delivery_pct = 100.0` where the SQL's
     `equity_id` order is the deterministic one.
   - F4's widening moves distribution_warning's candidate pool from **103 to
     277** stocks (zone in the neutral bands with `zone_rn10 = 'Strong Bull'`),
     consistent with the 17/25 membership drift.

   **Owner action:** run it in pgAdmin, then
   `REFRESH MATERIALIZED VIEW km_scan_results;` followed by
   `REFRESH MATERIALIZED VIEW km_scan_exclusion_counts;` (plain REFRESH first —
   CONCURRENTLY needs existing data).
