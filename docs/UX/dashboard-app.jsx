...
function App() {
  ...
  const [density, setDensity] = usS(() => localStorage.getItem('dq.density') || 'standard');
  const [tourStep, setTourStep] = usS(() => localStorage.getItem('dq.tourSeen') ? -1 : 0);
  const [floorOpen, setFloorOpen] = usS(false);
  const [rotationOpen, setRotationOpen] = usS(false);

  usE(() => { localStorage.setItem('dq.density', density); }, [density]);

  const showTransits   = density !== 'calm';
  const showOutlook    = density !== 'calm';
  const showPanchangam = density !== 'calm';
  const showFloor      = density === 'terminal' || floorOpen;
  const showRotation   = density === 'terminal' || rotationOpen;
  const floorCollapsible = density !== 'terminal';
  const rotationCollapsible = density !== 'terminal';

  const gridCols = density === 'calm'
    ? '1fr'
    : '320px 1fr 320px';

  const endTour = () => { localStorage.setItem('dq.tourSeen', '1'); setTourStep(-1); };
  const replayTour = () => { setTourStep(0); };

  return (
    <React.Fragment>
      <TopBar density={density} setDensity={setDensity} onReplayTour={replayTour}/>
      <TodaysSky/>

      <div className="main-grid" style={{display:'grid', gridTemplateColumns: gridCols, gap:16}}>
        {showTransits && <CurrentSky/>}
        <div>
          <PingsColumn/>
        </div>
        {(showOutlook || showPanchangam) && (
          <div className="outlook-col">
            {showOutlook && <SixDayOutlook/>}
            {showPanchangam && <PanchangamSide/>}
          </div>
        )}
      </div>

      {floorCollapsible && !floorOpen && (
        <CollapsedRevealer
          label="THE MARKET FLOOR"
          note="Breadth · Momentum · Rotation · Leadership"
          onOpen={() => setFloorOpen(true)}
        />
      )}
      {showFloor && <AmbientGauges expanded={gauge} onExpand={setGauge}/>}

      {rotationCollapsible && !rotationOpen && (
        <CollapsedRevealer
          label="SECTOR ROTATION · INDUSTRY FLOW"
          note="167 industries · capital flow graph"
          onOpen={() => setRotationOpen(true)}
        />
      )}
      {showRotation && <SectorRotation/>}
      ...
      {tourStep >= 0 && <VaNiTour step={tourStep} setStep={setTourStep} onEnd={endTour} density={density} setDensity={setDensity}/>}
    </React.Fragment>
  );
}
