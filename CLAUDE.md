...
Next migration number: **171**. (170 = `km_migration_170_scan_matview_refresh_logic.sql` — rebuilds the scanner matviews to match the scanners the app actually runs: smart_money's industry gate (absolute `>60`, which returned ZERO rows on every refresh, → the 2026-07-19 `>55 OR top-decile`), distribution_warning's zone handling (7-band coercion + IN-list, so `Neutral Bull`/`Neutral Bear` stop being coerced to NULL), the `latest` CTE (row-count → `ema_20 IS NOT NULL`), and drops retired fresh_breakout. DROP/CREATE, so it re-applies 151's MAINTAIN grants + 157's kd_app ownership — without those the nightly `scan_refresh` fails with permission denied. Verified against live 2026-08-14 data before shipping; re-run `scripts/qa/scan-parity` after applying. See `docs/claude/scan-matview-audit-2026-08-15.md`; 169 = cadence-aware indicator warm-up; 168 = Volume Drive preset; 167 = NSE value_cr rescale; 166 = `km_migration_166_golarambh_almanac.sql` — Golārambha family: 4 generator-fed `planet_state` Sun rules (Uttara/Dakshina Gola halves + equinox ±1d turn windows, tag 'Gola'), windows from `scripts/generate_golarambh_windows.py` (TROPICAL equinox crossings — deliberately not the sidereal sankranti), almanac body in AlmanacPage + `astro_group:Gola` overlay; 165 = force-reonboard theme; 164 = forgot-password token leak; 163 = pricing GST beta default; NOTE 161/162 have DUPLICATE numbers (rule_evidence + scan_presets at 161, rule_evidence_transitions + user_bookmarks at 162); 160 = Mercury-slice launch catalog scope; see `docs/claude/astro-story.md`. ⚠ Numbering drifted: duplicates also at 152/153 and no 155 — always `ls App/DBscripts/ | sort` before picking a number, don't trust this line alone.)
...
### 📋 FOR REVIEW (owner) — Weekly/Monthly scanner activation
`docs/claude/weekly-monthly-scanners.md` (2026-08-16) — how to turn on weekly/monthly scanners. The plumbing already exists (`executeScan(…, timeframe)`, `loadWeeklyOrMonthlyBundle`, `kd_scan_presets.timeframe`, both aggregate tables populated nightly) but **no preset is configured for either timeframe** — all 14 active presets are `timeframe='daily'`. **Not a backfill problem**: of 777 stocks with >800 daily bars, all 777 have ≥145 weekly bars — weekly aggregation is complete wherever daily history supports it. Four separate blockers, measured against live data: **B1** the frontend mapper hardcodes `sma_150`/`sma_50`/`rss_value`/`w52_low`/`accum_distrib` to null even though `km_equity_weekly` HAS them (~20-line fix); **B2** long-window indicators are depth-limited (`sma_150`/`accum_distrib`/long `magic_rs_zone` ≈ 23-26% weekly, 0% monthly) because ~1,551 stocks hold only 2-3 yr of daily history — design around it using `magic_rs_short_zone` (88% weekly) and `sma_50` (86%); **B3** `ret_*`/`lifetime_high` are 0% despite existing, and `stage`/`is_vani_*`/`avg_amt_*`/`delivery_surge_x`/`breakout_level`/`score_5d`/`dot_*` have no column at all — this is what blocks the Stage family, Breakout Surge and conviction_flow; **B4** `km_industry_eod` is daily-only, so the four industry-gated scanners would judge weekly signals against a daily-clock rotation. Per-scanner matrix in the doc: exactly ONE scanner (`distribution_warning`) is a genuine drop-in on weekly, `smart_money` needs only B4, everything else needs B3. **Monthly is not viable for any current scanner** (zone 24%, rvol 22%, sma_50 22%) — recommend deferring. Owner decisions needed: which weekly presets actually matter (if it's Stage/Breakout, skip Stage 1 and go straight to B3), whether monthly is wanted, and whether industry gates are required in a first cut. Independent of the matview repoint — do not block either on the other.
