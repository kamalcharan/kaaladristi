...
| Walk mode — design & implementation | **Button currently hidden** in `CorrelationPage.tsx` (line 824, wrapped in `{false && ...}`) pending design review and blocker resolution. **Design decision needed**: Should Walk mode be inline within CorrelationPage (scrub instances, update correlation detail card live), or navigate to `/workspace?walk=true` with a dedicated walkthrough state? **Tech blockers**: 1) **TradingChart subscription missing** — TradingChart.tsx accepts `onVisibleRangeChange` callback but does NOT import or subscribe to `chartSyncStore.visibleFrom/visibleTo`. Cannot respond to range changes initiated by parent. Must add subscription in TradingChart line 72. 2) **No walk state** exists in either `frameworkStore` or `chartSyncStore`. Need to add `walkInstance`, `walkInstanceIndex`, `setWalkInstance()`, `setWalkInstanceIndex()` to one of these stores (or create dedicated walkStore). 3) **WorkspaceChart.tsx line 13** — props interface only accepts `instrument: InstrumentRef`. No `walkInstance` prop. Must extend interface and pass through to TradingChart. 4) **Navigation target unclear** — CorrelationPage onClick handler (line 826) currently does nothing for paid users (`canWalk ? undefined : ...`). Must decide route and wire it. **Ready-to-use components**: TimelineSlider.tsx exists (179 lines, `src/components/domain/VisualPulse/TimelineSlider.tsx`), ready to integrate into Walk view. **Tier gate**: WALK_TIERS = `['trial', 'quarterly', 'annual', 'beta']` already wired in CorrelationPage line 448 and InlineGate line 938. **Next steps**: 1) Design review: inline vs navigate; 2) Add chartSyncStore subscription to TradingChart; 3) Add walk state to chosen store; 4) Wire button onClick to navigation/state setter; 5) Extend WorkspaceChart.tsx to accept and forward walkInstance. | After UX sprint |
