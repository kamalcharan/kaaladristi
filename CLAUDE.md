...
Latest migration: **144** (`km_migration_144_create_user_role.sql` — **the actual root cause** behind the Constituents/Flow Map (and all PostgREST-direct pages) breaking for logged-in non-admins: the `user` DB role was NEVER created. `km_profiles.role` is `NOT NULL DEFAULT 'user'`, and `kd_auth_login` (096/140) stamps the JWT `role` claim with the profile role, so PostgREST does `SET ROLE "user"` on every non-admin request — but no `CREATE ROLE` ever ran, so every such request failed (`role "user" does not exist`). Admin worked because the `admin` role exists (created manually on the VPS); FastAPI-fed pages worked because they use direct psycopg2, not PostgREST role-switching. Migration 144 creates `"user"` as a NOLOGIN INHERIT member of `authenticated` (inherits all current+future SELECT/EXECUTE/USAGE grants) and grants `"user"` to `authenticator` so PostgREST can `SET ROLE` into it. This makes the guarded grants in 137/142/143 moot — those all assumed the role existed and were silent no-ops. migration 143 is `km_migration_143_user_role_grant_sweep.sql` — systematic sweep for the missing-`user`-grant class: for every table/view/matview in `public` that role `authenticated` can already SELECT, grants SELECT to profile roles `"user"` + `admin`. Root cause: the historical blanket grant script listed anon/authenticated/kd_app/admin/kd_readonly but never `user`, and migration 096 made PostgREST run logged-in users as `user` — so any such table is readable anonymously + by admin but denied to logged-in non-admins. Mirrors `authenticated`'s read access only (never grants more; RLS row-scoping stays intact); supersedes the one-table-at-a-time patches 137 + 142. migration 142 is `km_migration_142_index_constituents_grants.sql` — grants `SELECT ON km_index_constituents` to profile roles admin/"user" (+ authenticated/anon/kd_app defensively): fixes Sector Rotation → index detail Constituents + Flow Map tabs showing "Unable to load flow data"/empty constituent list for regular users while admin saw them fine. Same bug class as migration 137 — the table was created in migration 022 with RLS + a permissive read policy (`USING (true)`) but ZERO table-level GRANTs, and migration 096 made PostgREST execute logged-in users as DB role `user` (not `authenticated`), so `user` got permission-denied. migration 141 is `km_migration_141_profile_mode.sql` — `km_profiles.mode TEXT` (dark/light/system, default 'dark'): makes color-mode preference follow a user across devices the same way `theme` already does via migration 091 — `updateProfile({ mode })` on change in ThemeSettings, re-applied via `applyProfileTheme()` in authStore on login/session restore; supersedes the never-wired `dark_mode BOOLEAN` column from migration 091, left in place unused; migration 140 is `km_migration_140_users_admin.sql` — Users admin: `km_profiles.is_suspended` + `kd_auth_login` rejects suspended accounts at next login + `km_admin_audit` action log; feeds the admin-only /users page (list, suspend, plan reassign, subscription extend, physical delete via `/api/admin/users/*` FastAPI endpoints with server-side role check); migration 139 is `km_migration_139_confidence_benchmarks.sql` — `km_rule_confidence_bench`: per-benchmark rule validation, PK (rule_id, benchmark_index_id); windows stay universal, `score_benchmark_confidence()` in confidence_scoring.py measures each window on each index's closes (≥250 bars, curated included) vs the active-inference hypothesis; runs nightly 19:00, on Compute Confidence, and per-rule on inference save/delete; read by the chart tooltip (viewed index's row, NIFTY 50 fallback) and /rules/:id BenchConfidenceStrip; migration 138 is `km_migration_138_confidence_hypothesis.sql` — km_rule_confidence gains `hypothesis_source` ('inference'|'base_bias') + `hypothesis_impact`: records WHICH hypothesis the matched/confidence numbers were tested against; stamped by `rescore_rules()` in confidence_scoring.py, which re-derives `matched` from stored returns × the current hypothesis on inference save, inference delete, nightly 19:00 scoring, and manual Compute Confidence; migration 137 is `km_migration_137_patterns_grants_profile_roles.sql` — grants km_rule_patterns/km_rule_inference to profile roles admin/user, the JWT roles PostgREST actually runs as; migration 136 is `km_migration_136_inference_versioning.sql` — km_rule_inference versioning: one ACTIVE hypothesis per (rule_a, rule_b) scope, auto-supersede on save with frozen validation snapshot; migration 135 widens `km_rule_inference` to the full /inference capture shape: 12-value market_impact vocabulary, expert `confidence` 1-10, `applicability_scope`/`applicability` JSONB, `notes`; migration 134 created the table, 133 restricts Catalog visibility to Mercury/Mars/Saturn/Jupiter/Bayer/MajorTransit-tagged rules)
...
- **PostgREST — the `user` DB role must actually EXIST**: the deeper half of the above, found 2026-07-09. `km_profiles.role` is `NOT NULL DEFAULT 'user'`, and `kd_auth_login` stamps the JWT `role` claim with the profile role, so PostgREST does `SET ROLE "user"` for every logged-in non-admin. But no migration ever ran `CREATE ROLE "user"` — it did not exist, so EVERY PostgREST-direct request from a non-admin failed (`role "user" does not exist`), not just un-granted tables. Admin worked (the `admin` role was created manually on the VPS); FastAPI-fed pages worked (direct psycopg2, no role-switching). Because 137/142/143 guarded their grants with `IF EXISTS (pg_roles WHERE rolname='user')`, they were silent no-ops the whole time. Fixed in migration 144, which creates `"user"` as a NOLOGIN INHERIT member of `authenticated` and grants it to `authenticator`. Diagnostic one-liners: `SELECT rolname FROM pg_roles WHERE rolname='user';` (must return a row) and `SELECT has_table_privilege('user','public.<table>','SELECT');` (must not error). Since `"user"` now inherits `authenticated`, the "forgot to grant the user role" class is permanently closed — new tables granted to `authenticated` auto-flow to `"user"`.
