...
### ⚠ PEAD's Drift column could never populate — it is computed in the scanner (2026-09-24)

`kd_result_returns` fills `drift_pct` / `sessions_elapsed` / `end_close` **only
once the FULL horizon has passed**. The Post-Result Drift scanner's membership
rule is *"still inside the 20-session window"* — so **every row it can ever
return has a NULL `drift_pct`**, and the shipped column read `—` on every stock,
always. Not a styling gap: a structural one.

`fetchPeadDrift` now derives both:

- **drift so far** = latest close ÷ **`day_0_close`** − 1. ⚠ **Never
  `base_close`** — that is Day −1, and measuring from there folds the
  announcement jump into the drift, the exact error migration 215's header
  exists to prevent. Reaction (D−1→D0) and drift (D0→now) stay separate.
- **sessions elapsed** = the Day 0 date's index in the 21-session calendar
  already fetched (`dates.indexOf`), never date arithmetic — a holiday would
  mis-count the window silently.
- A missing or zero `day_0_close` yields **null**, never a division. `d0Close ?? 1`
  looks like a guard and is not: it fabricates "drift = close − 100%" from a
  missing anchor. A test pins the divisor.

Verified live on 2026-09-23: SKYWAYS, Day 0 18 Sept, base 117.98 → D0 124.43
(reaction **+5.47%**) → latest 129.57 = **drift so far +4.13%, 3 / 20 sessions**,
while the RPC's own `drift_pct` is NULL. The column header says **"Drift so
far"**, because every row on this list is a partial reading and the Sessions
column states how partial.

⚠ **`magic_rs` / `rvol` reading `—` and `stage` reading `UNKNOWN` are NOT bugs**
— checked on the live bar, SKYWAYS genuinely carries NULL for both (a recent
listing). ⚠ `MCAP MIN 100` in the filter bar is `DEFAULT_FILTERS`, the same
default the main Scanner page applies; it is visible in the Filters count and
was left alone rather than special-cased.

**The seasonal empty state is wired** (`EMPTY_COPY` in `ScanView`): *"No results
filed in the last 20 sessions"*, never "no stocks match" — 104 names in the week
of 2026-08-10 against **one** on 2026-09-23, so this list is empty four weeks in
five by design and the generic line reads as a broken screen. Both empty
branches take head AND body; the test counts the `??` fallbacks rather than the
lookups, because swapping `??` for `&&` keeps the lookup count identical and
blanks the line.

Guarded by `scripts/qa/check-pead-scanner.mjs` (11 checks), verified to fail
against ten sabotages including drift read back from the RPC, drift measured
from `base_close`, the calendar replaced by a constant, the zero-close guard
...
- **A naive `datetime.utcnow()` into a `timestamptz` column is stored in the SESSION's timezone, and the guard it silently disables is the one you added after the last outage** (2026-09-25). `pipeline2/worker.py` wrote `km_jobs.completed_at` from `datetime.utcnow()`. psycopg2 sends a naive value as a bare timestamp literal; PostgreSQL resolves a bare literal against the session `TimeZone`, which is `Asia/Kolkata`, so every completion instant was stored **exactly 19,800s (5h30m) early** — measured, and reproduced on a throwaway cluster against both forms side by side. **4,274 of 4,296 rows** carried `completed_at` EARLIER than their own `started_at`. `started_at` was always right, because `_claim_job` writes SQL `now()` — which is what makes the bug so easy to miss: the table has one correct column and one wrong one, written five lines apart. **The damage was not the wrong timestamps.** Two guards window on `completed_at > now() - interval`: `_parent_fix_failed` (120 min — the deferral added on 2026-09-23 precisely so a dependent would stop running against columns its parent's failed forced fix had nullified) and the cascade debounce (30 min). A row 5.5 hours early cannot fall inside either, so **both were inert from the day they shipped** — zero jobs have ever reached `status='deferred'`, including on 2026-09-24 when three fixes failed at 19:30 and the cascade ran at 20:05, inside the window it was written to catch. Its unit tests passed throughout, because they build their own rows. Three things worth carrying: (1) **let the database stamp the database's own rows** — the fix is a `SQL_NOW` sentinel that `_update_job` renders as `now()`, not an aware Python datetime, because an aware datetime is one deleted argument from being naive again; (2) **a guard that has never fired is not evidence that it works** — `SELECT count(*) … WHERE status='deferred'` was the whole diagnosis, and any new guard deserves that query; (3) **repairing timestamps needs a band, not a sign test** — the residual after shifting is a few seconds of app-host-vs-database clock drift, so `WHERE completed_at < started_at` would re-shift those rows by another 5h30m on a re-run (migration 225 uses a 3–6 hour band and is verified idempotent over three passes). ⚠ `km_dimension_watermarks.computed_at` was checked and is NOT affected — it already uses `now()`.
