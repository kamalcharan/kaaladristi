...
- **Backend DELETE**: endpoint exists but is no longer used by the frontend — DELETE was unreliable when item IDs contain colons (e.g. `astro_rule:CODE`) due to URL encoding edge cases.
...
- **Force refresh** (admin "clear cache" button): `queryClient.removeQueries(['corr-insight', itemA, itemB])` evicts all variants, then `setRefreshCount(c => c + 1)` makes the next POST send `force_refresh: true` — backend skips its cache and calls LLM directly. `setVaniTriggered(false)` so user must re-click "Ask VaNi".
- **Manual-only trigger**: `vaniTriggered` state — starts `false`, set `true` only on "Ask VaNi" button click.
